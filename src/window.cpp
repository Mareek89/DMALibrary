...
void window::render_frame() {
    ...
    if (render_hook != NULL) {
        reinterpret_cast<void(*)()> (render_hook)();
    }
...
void window::set_mouse_passthrough(bool state) {
    ...
    if (state) {
        SetWindowLongA(hwnd, -20, (LONG_PTR)(GetWindowLongA(hwnd, -20) & ~0x20));
    } else {
        SetWindowLongA(hwnd, -20, (LONG_PTR)(GetWindowLongA(hwnd, -20) | 0x20));
    }
}
...
void window::setup_graphics(HWND hwnd) {
    ...
    if (result != S_OK) {
		std::cout << xorstr_("[!] Failed to create device and swap chain: ") << int(result) << xorstr_("\n");
		return;
    }
...
void window::init(bool* running) {
    ...
    hwnd = CreateWindow(title, title, WS_OVERLAPPEDWINDOW, 0, 0, window::wx, window::wy, NULL, NULL, wc.hInstance, NULL);
    if (hwnd == NULL) {
		std::cout << xorstr_("[!] Failed to create window: ") << GetLastError() << xorstr_("\n");
		return;
    }
...
BOOL CALLBACK MonitorEnumProc(HMONITOR hMonitor, HDC hdcMonitor, LPRECT lprcMonitor, LPARAM dwData) {
    ...
    if (GetMonitorInfo(hMonitor, &monitorInfo)) {
        ...
        SetWindowLong(window::hwnd, GWL_EXSTYLE, WS_EX_TOPMOST);
...
font_data* window::load_font(std::string font, int sizeint, bool set_default, bool set_selected) {
	...
	std::cout << xorstr_("[!] Loading font: [") << path << xorstr_("] [") << size << xorstr_("px]\n");
